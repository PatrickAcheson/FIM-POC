...
def get_file_info(file_path):
    ...
    return {"path": str(file_path), "hash": generate_file_hash(file_path), "size": file_stats.st_size, "permissions": oct(file_stats.st_mode)[-3:]}
...
def save_baseline(file_path, baseline_data):
    with open(file_path, "w") as f: json.dump(baseline_data, f, indent=2)

# Function to log change in the system log file
def log_change(date, change_type, file_path):
    with open(f'sys_log_{date}', 'a') as f: f.write(f"{datetime.now().strftime('%Y-%m-%d_%H:%M:%S.%f')} {change_type} {file_path}\n")
...
class FileChangeHandler(FileSystemEventHandler):
    def __init__(self, baseline_data): self.baseline_data = baseline_data
    def on_modified(self, event): detect_changes(event.src_path, self.baseline_data, "File modified:") if not event.is_directory else None
    def on_created(self, event): detect_changes(event.src_path, self.baseline_data, "File created:") if not event.is_directory else None
    def on_deleted(self, event): detect_changes(event.src_path, self.baseline_data, "File deleted:") if not event.is_directory else None

# Function to monitor a directory
def monitor_directory(directory, baseline_data):
    observer = Observer()
    observer.schedule(FileChangeHandler(baseline_data), directory, recursive=True)
    observer.start()
    try:
        while True: pass
